...
public class Voiture {
	...
	public void entreAuGarage(Garage g) throws Exception {
		...
		if (myStationnements.size() > 0) {
			if (myStationnements.get(myStationnements.size() - 1).estEnCours()) {
				throw new Exception("Un véhicule est déjà stationné");
			}
	...
	public void sortDuGarage() throws Exception {
		...
		if (myStationnements.size() == 0) {
				throw new Exception("Le véhicule n'est pas stationné");

				if (!(myStationnements.get(myStationnements.size() - 1).estEnCours())) {
					throw new Exception("Le véhicule n'est pas en stationnement");
			}
	...
	public Set<Garage> garagesVisites() {
		Set<Garage> vstgarage = new HashSet<Garage>();
		for (int i = 0; i < myStationnements.size(); i++) {
			vstgarage.add(myStationnements.get(i).getGarage());
		}
		return vstgarage;
	}
	...
	public void imprimeStationnements(PrintStream out) {
		Set<Garage> vstgarage = garagesVisites();
		for (Garage garage : vstgarage) {
